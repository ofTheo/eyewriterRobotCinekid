...
class stroke{
	...
	public:
		...
		void addPoint(float x, float y, float t, float confidence, float cx, float cy){
			pts.push_back(strokePoint(x, y, t, confidence, cx, cy));
			updateBoundingRect();
		}
...
//				}else{
					...
					ofBeginShape();
						for(int i = 0; i < pts.size(); i++){
							...
							}else if( i > 0 && pts[i-1].bCurve ){
								ofBezierVertex(pts[i-1].cx, pts[i-1].cy, pts[i].cx, pts[i].cy, pts[i].x, pts[i].y);
							}
